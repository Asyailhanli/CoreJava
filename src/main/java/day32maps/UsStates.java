...
public enum UsStates {//enum lar sabit degerler koyulur icine
    ...
    public static  String getStateNameFromAbbreviation(String abbreviation){
         return Arrays.stream(values()).//enum daki butun data'yi String'in icine koyuyor
                 filter(UsStates->UsStates.getAbbreviation().equals(abbreviation)).//verdigim kurallara gore onlari eler
                 findFirst().//FLORIDA("Florida","FL","Tallahasse")
                 get().//sadece data type degistirmek icin get methodu kullaniliyor ozellikle findFirst ile birlikte kullaniliyor
    ...
    //Example 1: Baskenti verilen state'nin kisaltmasini ekrana yazdiran method'u olusturunuz
